...
5. 松开鼠标后进入编辑模式，可以：
   - 拖拽四角/四边手柄调整选区大小
   - 拖拽选区内部移动位置
   - 点击工具栏的「确认识别」「重选」「取消」按钮
   - 按 Enter 确认或 ESC 取消
6. 确认后等待识别完成
7. 在弹出的结果窗口中查看或复制识别到的文字
...
### v1.5.1 (2026-02-27)
- 🐛 **修复重选按钮bug** - 点击"重选"按钮后可以正常重新框选区域，不再出现选框立刻被框定的问题
- 🛡️ **优化事件处理** - 防止点击工具栏按钮时事件冒泡干扰框选操作

### v1.5.0 (2026-02-27)
- ✨ **新增选区编辑模式** - 框选后进入编辑模式，支持拖拽调整选区大小和位置
- 🎯 **8个调整手柄** - 四角和四边中点各有一个圆形拖拽手柄
- 📏 **工具栏** - 显示选区尺寸，包含确认/重选/取消按钮
- ⌨️ **键盘快捷键** - 编辑模式下 Enter 确认、ESC 取消

### v1.4.1 (2026-02-27)
- ✨ **优化选区提示** - 初始提示显示最小尺寸限制，拖拽时实时反馈选区大小，过小时显示红色警告
